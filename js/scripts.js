(function ($) {
  ...
  if (document.getElementById("header")) {
    document.getElementById("copyip").addEventListener("click", () => {
      navigator.clipboard.writeText("mc.maniamc.networkt").then((error) => {
        if (error) console.error(error);
        else
          Swal.fire({
            icon: "success",
            title: "IP do servidor copiado",
            html: "IP do servidor copiado com sucesso para a área de transferência.",
          });
      });
    });
  }
  ...
  initServerData("mc.maniamc.network", "28706");
})(window.jQuery);
